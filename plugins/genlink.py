...
async def gen_link_s(bot, message):
    ...
    await message.reply(f"https://telegram.dog/{temp.U_NAME}?start={outstr}")
...
async def gen_link_batch(bot, message):
    ...
    if len(links) != 3:
        return await message.reply("Use correct format.\nExample <code>/batch https://telegram.dog/TeamEvamaria/10 https://telegram.dog/TeamEvamaria/20</code>.")
    ...
    if chat_id in FILE_STORE_CHANNEL:
        ...
        return await sts.edit(f" https://telegram.dog/{temp.U_NAME}?start=DSTORE-{b_64}</b>")
